...
extern int pharmacy_tab;
extern int pharmacy_sel;

static void draw_tabs(App *app, Rect c){
  const char *t0 = pharmacy_tab==0 ? "[Meds]" : " Meds ";
  const char *t1 = pharmacy_tab==1 ? "[Lotes]" : " Lotes ";
  const char *t2 = pharmacy_tab==2 ? "[Relatórios]" : " Relatórios ";
  ui_print(c.y+3,c.x+2,pharmacy_tab==0?93:90,49,"%s",t0);
  ui_print(c.y+3,c.x+10,pharmacy_tab==1?93:90,49,"%s",t1);
  ui_print(c.y+3,c.x+19,pharmacy_tab==2?93:90,49,"%s",t2);
...
void pharmacy_views_draw(App *app, Rect c){
  ...
  if(pharmacy_tab==0){
    ...
    for(int i=0;i<n && i< (c.h-10); i++){
      ...
      int fg=(i==pharmacy_sel)?97:90;
      int bg=(i==pharmacy_sel)?47:49;
